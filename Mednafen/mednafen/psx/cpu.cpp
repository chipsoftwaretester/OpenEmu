...
PS_CPU::PS_CPU()
{
 ...
 memset(DummyPage, 0x00, sizeof(DummyPage));

 dp_offset = SetupFastMapTrampoline(DummyPage, FAST_MAP_PSIZE);

// for(uint64 a = 0x00000000; a < (1ULL << 32); a += FAST_MAP_PSIZE)
//  SetFastMap(&DummyPage[dp_offset], a, FAST_MAP_PSIZE);
...
uint32 PS_CPU::SetupFastMapTrampoline(void *alloced_mem, uint32 region_size) // TODO
{
 return(FAST_MAP_TRAMPOLINE_SIZE);
}
...
uint32 PS_CPU::Exception(uint32 code, uint32 PC)
{
 ...
 {
  printf("Exception: %08x @ PC=0x%08x %d -- IPCache=0x%02x -- IPEND=0x%02x -- SR=0x%08x ; IRQC_Status=0x%04x -- IRQC_Mask=0x%04x\n", code, PC, InBDSlot, IPCache, (CP0.CAUSE >> 8) & 0xFF, CP0.SR,
...
pscpu_timestamp_t PS_CPU::RunReal(pscpu_timestamp_t timestamp_in)
{
 ...
 register uint32 PC = BACKED_PC;
 register uint32 new_PC = BACKED_new_PC;
 register uint32 new_PC_mask = BACKED_new_PC_mask;
...
*/
   instr = *(uint32 *)&FastMap[PC >> FAST_MAP_SHIFT][PC];
...
meowmoo &= 1;
...
#if 0
   #define DO_BRANCH(branch_target)		\
	{					\
	 uint32 bt_tmp = (branch_target);	\
         uint32 new_PC_save = new_PC;		\
						\
         if(DebugMode && ADDBT)                 \
          ADDBT(new_PC, bt_tmp, false); /*+4 or not in from... */	        \
						\
         new_PC = bt_tmp;			\
						\
         if(InBDSlot)				\
          PSX_WARNING("[CPU] Branch in delay slot at PC=%08x, next PC=%08x, new_PC=%08x", PC, new_PC_save, new_PC);	\
						\
         PC = new_PC_save;			\
						\
         InBDSlot = true;			\
...

   {
    BEGIN_OPF(ILL, 0, 0);
	     ...
	     DO_LDS();
	     new_PC = Exception(EXCEPTION_RI, PC);
	     new_PC_mask = 0;
    END_OPF;
	...
	if(ep)
	{
	 new_PC = Exception(EXCEPTION_OV, PC);
         new_PC_mask = 0;
	}
	else
	 GPR[rd] = result;
	...
	DO_LDS();
	new_PC = Exception(EXCEPTION_BP, PC);
        new_PC_mask = 0;
    END_OPF;
	...
	DO_LDS();
        new_PC = Exception(EXCEPTION_COPU, PC);
        new_PC_mask = 0;
    END_OPF;
    ...
    BEGIN_OPF(LWC2, 0x32, 0);
        ...
        if(address & 3)
	{
         new_PC = Exception(EXCEPTION_ADEL, PC);
         new_PC_mask = 0;
	}
        else
	{
	...
	if(address & 0x3)
	{
	 new_PC = Exception(EXCEPTION_ADES, PC);
         new_PC_mask = 0;
	}
	...
	new_PC = Exception(EXCEPTION_SYSCALL, PC);
        new_PC_mask = 0;
    END_OPF;
	...
	else
	{
	 ...
	 new_PC = Exception(EXCEPTION_INT, PC);
         new_PC_mask = 0;
	}
    END_OPF;
   }
