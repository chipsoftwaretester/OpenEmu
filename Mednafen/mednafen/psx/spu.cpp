...
/* TODO:
	Proper IRQ semantics; output continuous level instead of pulse, support program ack, see if there's an IRQ asserted SPU status bit.
	...
	Make sure SPU IRQ works correctly, test to see if it should be triggered on SPU RAM writes and/or reads by the CPU.
...
void PS_SPU::DecodeSamples(SPU_Voice *voice)
{
 ...
 if(flags & 0x4)
 {
  //PSX_WARNING("[SPU] Flag loop set for voice %d, 0x%08x", (int)(voice - Voices), PrevCurAddr);
...
INLINE void PS_SPU::WriteSPURAM(uint32 addr, uint16 value, bool test_irq)
{
 if(test_irq)
  CheckIRQAddr(addr);
...
//pscpu_timestamp_t PS_SPU::Update(const pscpu_timestamp_t timestamp)
{
 ...
 while(sample_clocks > 0)
 {
  ...
  for(int voice_num = 0; voice_num < 24; voice_num++)
  {
   ...
   // Increment stuff
   {
    int32 phase_inc = voice->Pitch;

    if(PhaseModCache & (1 << voice_num))
    {
     phase_inc = (voice->Pitch * ((voice - 1)->PreLRSample + 0x8000)) >> 15;
  ...
  if(!(SPUControl & 0x4000)) // General output muted? TODO: Determine if it affects CD audio too; and if not, and we add sub-sample timing accuracy,
			     // if it's checked for every channel at different times, or just once.
  {
   accum_l = 0;
   accum_r = 0;
   accum_fv_l = 0;
...
{
 //PSX_WARNING("[SPU] memWrite: %08x %08x\n", RWAddr, V);

 WriteSPURAM(RWAddr, V, false);
 RWAddr = (RWAddr + 1) & 0x3FFFF;

 WriteSPURAM(RWAddr, V >> 16, false);
 RWAddr = (RWAddr + 1) & 0x3FFFF;
}
...
void PS_SPU::Write(pscpu_timestamp_t timestamp, uint32 A, uint16 V)
{
 ...
 if(A < 0x180)
 {
  ...
  switch(A & 0xF)
  {
   ...
   case 0x04: voice->Pitch = V & 0x3FFF;
	      break;
   ...
   case 0x0E: voice->LoopAddr = (V << 2) & 0x3FFFF;
	      //PSX_WARNING("[SPU] Manual loop address setting for voice %d: %04x", voice - Voices, V);
	      break;
  }
 }
 else
 {
  switch(A & 0x7F)
  {
   ...
   case 0x24: IRQAddr = (V << 2) & 0x3FFFF;
	      //PSX_WARNING("[SPU] IRQ addr reg 0x2c set: 0x%04x", V);
	      break;

   case 0x26: RWAddr = (V << 2) & 0x3FFFF;
	      break;

   case 0x28: WriteSPURAM(RWAddr, V, false);
	      RWAddr = (RWAddr + 1) & 0x3FFFF;
	      break;
